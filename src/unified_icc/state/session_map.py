...
from ..state.state_persistence import unwired_save
from ..utils.utils import atomic_write_json
...
class SessionMapSync:
    ...
    def _sync_window_from_session_map(
        ...
        *,
        ...
        if (
            new_wname
            and channel_router.get_display_name(window_id) == window_id
            and not state.window_name
        ):
            state.window_name = new_wname
            channel_router.set_display_name(window_id, new_wname)
            changed = True
        return changed
