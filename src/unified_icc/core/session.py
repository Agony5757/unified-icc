...
Manages the key mappings:
  ...
  User→Thread→Window: delegated to ThreadRouter (see channel_router.py).
...
Thread routing: delegated to ThreadRouter (see channel_router.py) — no pass-throughs.
"""
...
from ..tmux.window_resolver import EMDASH_SESSION_PREFIX, is_foreign_window, is_window_id
from ..tmux.window_view import WindowView
from ..tmux.window_state_store import (
    APPROVAL_MODES,
    BATCH_MODES,
...
def _migrate_mailbox_ids(
    ...
    new_display = {
        wid: channel_router.window_display_names.get(wid, "") for wid in new_states
    }
...
class SessionManager:
    ...
    Thread routing (channel_bindings, display names, group_chat_ids) is
    delegated to ThreadRouter — see channel_router.py.
    ...
    def window_display_names(self) -> dict[str, str]:
        return channel_router.window_display_names
    ...
    def _wire_singletons(self) -> None:
        ...
        channel_router._schedule_save = self._save_state
        channel_router._has_window_state = lambda wid: wid in window_store.window_states
    ...
    async def resolve_stale_ids(self) -> None:
        ...
        """
        from ..tmux.window_resolver import LiveWindow, resolve_stale_ids as _resolve
        ...
        if changed:
            channel_router._rebuild_reverse_index()
            self._save_state()
    ...
    def set_display_name(self, window_id: str, window_name: str) -> None:
        ...
        channel_router.set_display_name(window_id, window_name)
    ...
    def sync_display_names(self, live_windows: list[tuple[str, str]]) -> bool:
        ...
        router_changed = channel_router.sync_display_names(live_windows)
    ...
    def prune_stale_state(
        ...
        stale_display = [
            wid
            for wid in channel_router.window_display_names
        ...
        stale_chat = (
            []
            if skip_chat_ids
            else [k for k in channel_router.group_chat_ids if k not in bound_keys]
        )
    ...
    def audit_state(
        ...
        for wid in channel_router.window_display_names:
            if wid not in live_window_ids and wid not in in_use:
                name = channel_router.get_display_name(wid)
                issues.append(
                    AuditIssue(
        ...
        for wid, tmux_name in live_windows:
            stored_name = channel_router.window_display_names.get(wid)
