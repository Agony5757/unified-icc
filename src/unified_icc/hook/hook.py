...
Called by Claude Code hooks (SessionStart, Notification, Stop, etc.) to
maintain a window↔session mapping and an append-only event log.
...
_CURRENT_HOOK_MARKER = "unified_icc.hook"
_CCGRAM_HOOK_MARKER = "ccgram.main hook"
...
def _current_hook_command() -> str:
    return f"{shlex.quote(sys.executable)} -m unified_icc.hook"
...
def _is_any_known_hook_command(command: str) -> bool:
    return any(
        marker in command
        for marker in (_CURRENT_HOOK_MARKER, _CCGRAM_HOOK_MARKER, _PATH_HOOK_MARKER, _LEGACY_HOOK_MARKER)
    )
    ...
    for entry in event_hooks:
        ...
        for h in entry.get("hooks", []):
            if not isinstance(h, dict):
                continue
            if predicate(h.get("command", "")):
                return True
    return False


def _has_known_hook(settings: dict, event_type: str) -> bool:
    return _has_matching_hook(settings, event_type, _is_any_known_hook_command)


def _is_hook_installed(settings: dict) -> bool:
    return _has_known_hook(settings, "SessionStart")


def get_installed_events(settings: dict) -> dict[str, bool]:
    return {event: _has_known_hook(settings, event) for event in _HOOK_EVENT_TYPES}
...
def _install_hook() -> int:
    ...
    for event_type in _HOOK_EVENT_TYPES:
        ...
        has_known = _has_matching_hook(settings, event_type, _is_any_known_hook_command)

        if has_known and not has_current:
            _replace_hook_commands(
                settings, event_type, _is_any_known_hook_command, current_command,
            )
            installed_count += 1
            continue
...
def _uninstall_hook() -> int:
    ...
    any_installed = any(
        _has_known_hook(settings, event) for event in _HOOK_EVENT_TYPES
    )
    ...
    for event_type in _HOOK_EVENT_TYPES:
        ...
        for entry in event_hooks:
            ...
            filtered = [
                ...
                if not isinstance(h, dict)
                or not _is_any_known_hook_command(h.get("command", ""))
            ]
            if filtered:
...
def _extract_notification_data(payload: dict[str, Any]) -> dict[str, Any]:
    return {"tool_name": payload.get("tool_name", ""), "message": payload.get("message", "")}


def _extract_stop_data(payload: dict[str, Any]) -> dict[str, Any]:
    return {"stop_reason": payload.get("stop_reason", ""), "num_turns": payload.get("num_turns", 0)}


def _extract_stop_failure_data(payload: dict[str, Any]) -> dict[str, Any]:
    return {"error": payload.get("error", ""), "error_details": payload.get("error_details", "")}
...
def _update_session_map(
    ...
    from ..utils.utils import unified_icc_dir, atomic_write_json
                ...
                if map_file.exists():
                    try:
                        ...
                        try:
                            ...
                            logger.warning("Corrupted session_map.json backed up to %s", backup)
                        except OSError:
...
def _process_hook_stdin() -> None:
    ...
    if event == "SessionStart":
        ...
        _update_session_map(
            session_window_key, session_id, cwd, window_name,
            transcript_path, tmux_session_name,
        )
        _write_event(
            event, session_id, session_window_key,
            {"cwd": cwd, "transcript_path": transcript_path, "window_name": window_name},
        )
        return
