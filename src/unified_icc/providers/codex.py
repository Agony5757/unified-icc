...
def _canonical_tool_name(name: str) -> str:
    """Map provider-native tool names to canonical names."""
...
class CodexProvider(JsonlProvider):
    ...
    def parse_terminal_status(
        ...
        *,
        ...
        interactive = _extract_codex_interactive_content(pane_text)
        if interactive:
            formatted = format_codex_interactive_prompt(interactive.content, interactive.name)
            return StatusUpdate(
                raw_text=formatted,
...
# ── Minimal interactive UI detection for Codex ──────────────────────────────

_CODEX_EDIT_PROMPT_RE = re.compile(r"(?i)do you want to make this edit")
_CODEX_ESC_RE = re.compile(r"(?i)^\s*esc to (cancel|exit)")
_CODEX_ENTER_RE = re.compile(r"(?i)^\s*enter to (confirm|select|continue|submit)")
_CODEX_CHECKBOX_RE = re.compile(r"^\s*[☐✔☒]")


def _extract_codex_interactive_content(pane_text: str) -> "_InteractiveContent | None":
    """Extract Codex interactive prompt content from pane text.

    Codex uses simpler interactive UIs than Claude — mainly edit confirmations,
    checkbox questions, and action hints near the bottom of the pane.
    """
    if not pane_text:
        return None

    lines = [line.rstrip() for line in pane_text.strip().splitlines()]
    if not lines:
        return None

    # Detect edit confirmation prompts
    for i, line in enumerate(lines):
        if _CODEX_EDIT_PROMPT_RE.search(line):
            # Find action hint below the prompt
            for j in range(i + 1, len(lines)):
                if _CODEX_ESC_RE.search(lines[j]) or _CODEX_ENTER_RE.search(lines[j]):
                    content = "\n".join(lines[i:j + 1]).strip()
                    if content:
                        return _InteractiveContent(content=content, name="AskUserQuestion")
            # No action hint found — still return the prompt
            content = "\n".join(lines[i:]).strip()
            if content:
                return _InteractiveContent(content=content, name="AskUserQuestion")

    # Detect checkbox questions (AskUserQuestion) — look near bottom of pane
    bottom_lines = lines[-15:] if len(lines) > 15 else lines
    for i, line in enumerate(bottom_lines):
        if _CODEX_CHECKBOX_RE.search(line):
            start = max(0, len(lines) - 15 + i - 2)
            content = "\n".join(lines[start:len(lines) - 5]).strip()
            if content:
                return _InteractiveContent(content=content, name="AskUserQuestion")

    # Bottom-up action hint detection
    for i in range(len(lines) - 1, -1, -1):
        if _CODEX_ESC_RE.search(lines[i]) or _CODEX_ENTER_RE.search(lines[i]):
            # Scan up for question context
            for j in range(i - 1, max(0, i - 20), -1):
                if lines[j].strip() and not lines[j].strip().startswith(("─", "-", " ")):
                    content = "\n".join(lines[j:i + 1]).strip()
                    if content:
                        return _InteractiveContent(content=content, name="AskUserQuestion")
            # Just the action hint
...
class _InteractiveContent:
    """Content extracted from a Codex interactive UI."""
